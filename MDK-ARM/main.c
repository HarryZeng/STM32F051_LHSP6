...
{  
    ...
    GPIO_StructInit(&gpio_init_structure);  
    //GPIOA                                                         //PA-0~3??ADC  
    gpio_init_structure.GPIO_Pin = PWM_Pin|PWM1_Pin|PWM2_Pin;  
    ...
    nvic_init_structure.NVIC_IRQChannelPriority = 1;                //优先级为0  
    ...
    nvic_init_structure.NVIC_IRQChannelPriority = 3;                //优先级为4
    ...
    timer_init_structure.TIM_Period = 20;                          //每300 uS触发一次中断,??ADC  
    ...
    TIM_TimeBaseStructInit(&timer_init_structure);                  //初始化TIM结构体  
		...
		timer_init_structure.TIM_ClockDivision = TIM_CKD_DIV1;          //系统时钟,不分频,24M  
    timer_init_structure.TIM_CounterMode = TIM_CounterMode_Up;      //向上计数模式  
    timer_init_structure.TIM_Period = 12;                          //每300 uS触发一次中断,??ADC  
    ...
    TIM_TimeBaseInit(TIM2, &timer_init_structure);  
		...
		TIM_OC1Init(TIM2,&timer_OCinit_structure);
...
/* TIM1 ??? ---------------------------------------------------
  ...
  /* Time ??????*/
  timer_init_structure.TIM_Prescaler = 23;
  timer_init_structure.TIM_CounterMode = TIM_CounterMode_Up;  /* Time ????????????*/
  timer_init_structure.TIM_Period = 12;
...
//				OUTSumADCValue += adc_dma_tab[0];
				...
				if(DMA_GetADC_Counter>24)
				{
//					DMA_GetADC_Counter=0;
//					OUTADCValue= OUTSumADCValue/25;
//					OUTSumADCValue = 0;
					OUTADCValue = adc_dma_tab[0];
					sample_finish = 1;
...
void IO_GPIO_INIT(void)
{
    ...
    gpio_init_structure.GPIO_Pin = COMPOUT_Pin;  
    gpio_init_structure.GPIO_Mode = GPIO_Mode_OUT;                  
    gpio_init_structure.GPIO_OType = GPIO_OType_PP;                
    gpio_init_structure.GPIO_Speed = GPIO_Speed_50MHz;              
    gpio_init_structure.GPIO_PuPd= GPIO_PuPd_UP;                    
		GPIO_Init(COMPOUT_GPIO_Port, &gpio_init_structure);
    ...
    gpio_init_structure.GPIO_PuPd= GPIO_PuPd_NOPULL;                    
		...
		//ATT_GPIO_Port
    gpio_init_structure.GPIO_Pin = ATT_Pin;  
    gpio_init_structure.GPIO_Mode = GPIO_Mode_OUT;                                
    gpio_init_structure.GPIO_Speed = GPIO_Speed_2MHz;              
    gpio_init_structure.GPIO_PuPd= GPIO_PuPd_NOPULL;                    
		GPIO_Init(ATT_GPIO_Port, &gpio_init_structure);
